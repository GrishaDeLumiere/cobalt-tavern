...
4. Загрузчик всё сделает сам: установит зависимости, поднимет REST API шлюз и откроет интерфейс в браузере.

### ⚙️ Конфигурация (Смена порта)

При первом запуске сервер автоматически создаёт файл `config.json` в корневой папке проекта. Если у вас занят стандартный порт или требуется изменить сетевые параметры, просто отредактируйте его:
...
* `port` — порт, на котором запускается веб-интерфейс и REST API (по умолчанию `8000`).
* `host` — сетевой интерфейс (`0.0.0.0` для доступа с других устройств в локальной сети, `127.0.0.1` для локального доступа).
* `autoOpenBrowser` — открывать ли вкладку браузера автоматически при запуске (`true` / `false`).

> *Примечание: также поддерживается передача порта через переменную окружения `PORT`.*
